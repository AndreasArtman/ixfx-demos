 ...
 */
const use = () => {
  const { bounds, thing, tilt, headTilt } = state;
...
const update = () => {
  ...
  for (const pose of poses.getRawPoses()) {
    ...
    a = - Math.abs(Bipolar.scale(a, tiltMin, tiltMax)); 
  ...
  if (counted > 0) {
    const angleAverage = angleTotal;
    const headAngleAvrage = headAngleTotal
  
    // Interpolate toward average of all poses
    tilt = interpolate(angleAmount, tilt, angleAverage);
    headTilt = interpolate(angleAmount, headTilt, headAngleAvrage);
  }

  // Push toward 0 (neutral)
  tilt = Bipolar.towardZero(tilt, tiltDecay);
  headTilt = Bipolar.towardZero(headTilt, tiltDecay);



  // Save
  saveState({ tilt, head:headTotal, headTilt});
};
...
const computeHead = (pose) => {
  const rightEar = MoveNet.Coco.getKeypoint(pose, `left_ear`);
  const distanse = rightEar
  return distanse
}

const computeHead1 = (pose) => {
  const leftEar = MoveNet.Coco.getKeypoint(pose, `right_ear`);
  const distanse1 = leftEar
  return distanse1
}

const computeHeadAngle = (pose) =>{
  const rightEarAngle = MoveNet.Coco.getKeypoint(pose, `right_ear`);
  const leftEarAngle = MoveNet.Coco.getKeypoint(pose, `left_ear`);
  const angleHead = Points.angle(leftEarAngle, rightEarAngle);
  return angleHead; 
}
